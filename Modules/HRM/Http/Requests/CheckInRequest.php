...
class CheckInRequest extends FormRequest
{
    ...
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            ...
            $lateAllowed = EmployeeAttendanceDay::where('employee_id', $user->employee->id)->where('day', $todayName)
                ->where('allow_late_checkin', true)->exists();
            $timeOver = now()->format('H:i:s') > '12:00:00';
            if (!$lateAllowed && $timeOver) {
                $validator->errors()->add('message', 'You are late to mark attendance.');
            }
