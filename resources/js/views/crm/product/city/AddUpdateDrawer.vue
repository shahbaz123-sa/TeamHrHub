...
const props = defineProps({
  ...
  editingCity: { type: Object, default: null },
});
...
watch(
  ...
  (val) => {
    resetForm();
    if (val && props.editingCity) {
      form.value = {...props.editingCity};
    }
  },
  { immediate: true }
);
...
<template>
  ...
  >
    <AppDrawerHeaderSection
      :title="form.id ? 'Edit City' : 'Add New City'"
            ...
            <VRow>
              ...
              <VCol cols="12" class="d-flex justify-end gap-4 mt-6">
                <VBtn type="submit" :loading="isLoading || props.loading">
                  {{ form.id ? 'Update City' : 'Create City' }}
                </VBtn>
