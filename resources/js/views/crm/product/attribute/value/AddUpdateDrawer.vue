...
const props = defineProps({
  ...
  editingAttributeValue: { type: Object, default: null },
  fetchingAttributes: Function,
...
const fetchAttributes = async (search = '') => {
  ...
  await props.fetchingAttributes(search, true)
...
const closeNavigationDrawer = () => {
  ...
  emit("update:editingAttributeValue", null);
  resetForm();
};
...
watch(
  ...
  (val) => {
    resetForm();
    if (val && props.editingAttributeValue) {
      form.value = {...props.editingAttributeValue};
    }
  },
  { immediate: true }
);
...
<template>
  ...
  >
    <AppDrawerHeaderSection
      :title="form.id ? 'Edit Attribute Value' : 'Add New Attribute Value'"
            ...
            <VRow>
              ...
              <VCol cols="12" class="d-flex justify-end gap-4 mt-6">
                <VBtn type="submit" :loading="isLoading || props.loading">
                  {{ form.id ? 'Update Attribute Value' : 'Create Attribute Value' }}
                </VBtn>
