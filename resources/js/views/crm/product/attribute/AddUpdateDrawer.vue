...
const props = defineProps({
  ...
  editingAttribute: { type: Object, default: null },
});
...
const closeNavigationDrawer = () => {
  ...
  emit("update:editingAttribute", null);
  resetForm();
};
...
watch(
  ...
  (val) => {
    resetForm();
    if (val && props.editingAttribute) {
      form.value = {...props.editingAttribute};
    }
  },
  { immediate: true }
);
...
<template>
  ...
  >
    <AppDrawerHeaderSection
      :title="form.id ? 'Edit Attribute' : 'Add New Attribute'"
            ...
            <VRow>
              ...
              <VCol cols="12" class="d-flex justify-end gap-4 mt-6">
                <VBtn type="submit" :loading="isLoading || props.loading">
                  {{ form.id ? 'Update Attribute' : 'Create Attribute' }}
                </VBtn>
