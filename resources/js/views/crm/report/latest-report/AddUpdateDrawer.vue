...
const props = defineProps({
  ...
  editingLatestReport: { type: Object, default: null },
});
...
const closeNavigationDrawer = () => {
  ...
  emit("update:editingLatestReport", null);
  resetForm();
};
...
watch(
  ...
  (val) => {
    resetForm();
    if (val && props.editingLatestReport) {
      form.value = {...props.editingLatestReport};
    }
  },
  { immediate: true }
);
...
const onSubmit = async () => {
  ...
  try {
    ...
    Object.entries(form.value).forEach(([key, value]) => {
      ...
      else if(key === 'attachment' && !(value instanceof File)) return
...
<template>
  ...
  >
    <AppDrawerHeaderSection
      :title="form.id ? 'Edit Latest Report' : 'Add New Latest Report'"
            ...
            <VRow>
              ...
              <VCol cols="12">
                <VFileInput
                  ...
                  :label="form.attachment ? 'Attachment (Leave empty to keep current)' : 'Attachment'"
