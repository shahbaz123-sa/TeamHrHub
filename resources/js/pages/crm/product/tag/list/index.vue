...
import AddUpdateDrawer from '@/views/crm/product/tag/AddUpdateDrawer.vue';
...
const fetchTags = async () => {
  loading.value = true
  const { data, meta } = await $api('/product/tags', {
    query: {
      q: searchQuery.value,
      page: page.value,
...
const saveTag = async (formData, id) => {
  ...
  try {
    ...
    if(id) {
      ...
      await $api(`/product/tags/${id}`, {
        method: 'POST',
        body: formData,
        headers: { Authorization: `Bearer ${accessToken}` }
      })
    } else {
      await $api('/product/tags', {
        method: 'POST',
        body: formData,
        headers: { Authorization: `Bearer ${accessToken}` }
      })
...
const confirmDelete = async () => {
  loading.value = true
  try {
    await $api(`/product/tags/${deleteTargetId.value}`, {
      method: 'DELETE',
      headers: { Authorization: `Bearer ${accessToken}` }
    })
    fetchTags()
    ...
    <VCard>
      ...
      <VCardText>
        <div class="d-flex justify-sm-space-between flex-wrap gap-y-4 gap-x-6 justify-start">
          ...
          <div class="d-flex align-center flex-wrap gap-4">
            ...
            />
            <VBtn
              v-if="hasPermission('product_tag.create')"
      ...
      >
        ...
        <template #item.actions="{ item }">
          <IconBtn v-if="hasPermission('product_tag.update')" @click="openDrawer(item)">
            <VIcon icon="tabler-edit" />
          </IconBtn>
          <IconBtn v-if="hasPermission('product_tag.delete')" @click="askDelete(item.id)">
