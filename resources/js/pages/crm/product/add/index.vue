...
const fetchSku = async () => {
  try {
    const { data } = await useApi(
      createUrl("/product/sku/generate"),
      {
        headers: {
...
const saveProduct = async () => {
  ...
  try {
    ...
    router.push({ name: "crm-product-list" });
  } catch (error) {
    console.error(error)
      ...
      >
        ...
        <div class="d-flex gap-4 align-center flex-wrap">
          ...
          <VBtn
            ...
            @click="saveProduct">Publish Product
          </VBtn>
        </div>
      </div>
          ...
          <VCard class="mb-6" title="Product Information">
            <VCardText>
              <VRow>
                ...
                <VCol cols="12" md="6">
                  <AppTextField label="Slug" v-model="form.wc_slug" :rules="[requiredValidator, slugRule]" />
                </VCol>
                <VCol cols="12" md="6">
              ...
              <VTable v-if="form.variations.length">
                ...
                <tbody>
                  <tr v-for="(v, idx) in form.variations" :key="idx">
                    ...
                    <td>
                      <VBtn icon color="error" @click="form.variations.splice(idx, 1)">
